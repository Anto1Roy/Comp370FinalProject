...
class FileOutputApp:
    def __init__(self, root, articles, url_list, start_index, end_index):
        ...
        self.input_label_display = tk.Label(
            root,
        ...
        self.next_button = tk.Button(
            root, text="Next", command=lambda: self.next_content(), state=tk.NORMAL
        )
        ...
        self.save_button1 = tk.Button(
            button_frame,
            text="Positive",
            command=lambda: self.save_to_button("Positive"),
            state=tk.NORMAL,
        )
        self.save_button1.pack(side=tk.LEFT, padx=20)

        self.save_button2 = tk.Button(
            button_frame,
            text="Negative",
            command=lambda: self.save_to_button("Negative"),
            state=tk.NORMAL,
        )
        self.save_button2.pack(side=tk.LEFT, padx=20)

        self.save_button3 = tk.Button(
            button_frame,
            text="Neutral",
            command=lambda: self.save_to_button("Neutral"),
            state=tk.NORMAL,
        )
    ...
    def save_to_button(self, button_number):
        try:
            ...
            if (
                self.current_article < len(self.articles) - 1
    ...
    def display_current_item(self):
        ...
        if (
            self.articles[self.current_article] == None
            or len(self.articles[self.current_article]) == 0
        ):
...
if __name__ == "__main__":
    ...
    csv_file_path = (
        Path(__file__).parent.parent.parent / "data/articles/results/results.csv"
    )
    ...
    json_content = json.load(
        open(
            ...
            / "data/articles/scraping_content/content_swift.json",
            "r",
        )
    )
